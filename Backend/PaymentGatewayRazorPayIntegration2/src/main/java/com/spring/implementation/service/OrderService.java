...
public class OrderService {
	...
	public Orders createOrder(Orders order) throws RazorpayException {
		System.out.println("Received order: amount=" + order.getAmount() + ", email=" + order.getEmail());
		if (order.getAmount() == null || order.getAmount() <= 0) {
	        throw new IllegalArgumentException("Invalid amount: " + order.getAmount());
	    }

	    int amountInPaise = order.getAmount() * 100; 
		JSONObject options = new JSONObject();
        options.put("amount", amountInPaise /*order.getAmount()*/);
